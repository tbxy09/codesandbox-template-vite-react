...
import { Card, CardContent, IconButton, Typography, Collapse, TextareaAutosize, useTheme } from '@material-ui/core';
      ...
      return (
          <Accordion collapsible >
            <AccordionItem value="item" >
              <AccordionHeader expandIconPosition='end' color='blue'>
                Accordion Header
              ...
              <AccordionPanel>
                {items.map((item, index) => (
                  ...
                  <p key={index} onClick={() => onSelect(item)}>{item}</p>
                ))}
              </AccordionPanel>
            </AccordionItem>
          </Accordion>
...
export const HistoryList = ({ items, onSelect }: HistoryListProps) => {
    return (
      <div className="history-list" style={{ backgroundColor: "#333333" }}>
        {items.map((item) => (
